...
module ContribMod.LayoutGroups ( -- * Usage
                              -- $usage
                              -- * Creation
                              group
                              -- * Messages
...
class GroupHook a l where
  mHook :: a -> Query (ModSpec l)
...
data GroupsMessage = ToEnclosing SomeMessage -- ^ Send a message to the enclosing layout
                   ...
                   | ModifyForce ModifySpec -- do not do any fixes
...
instance Show GroupsMessage where
    ...
    show (ModifyForce _) =  "ModifyForce {...}"
...
readapt :: (GroupHook (Groups l l2 Window) l)
    => Zipper Window
    -> Groups l l2 Window
    -> X (Groups l l2 Window)
readapt z g = do let mf = getFocusZ z
                     (seed', id:_) = gen $ seed g
                     g' = g { seed = seed' }
                 flip modifyGroupsX g' $ \a -> do
                     let a'  = a & mapZ_ (onZipper $ removeDeleted z)
                                 & filterKeepLast (isJust . gZipper)
                                 & findNewWindows (W.integrate' z)
                     fmap (focusGroup mf >>> onFocusedZ (onZipper $ focusWindow mf))
                            (addWindows (ID id $ baseLayout g) (mHook g) a')
...
addWindows :: WithID l Window
    -> Query (ModSpec l)
    -> (Zipper (Group l Window), [Window])
    -> X (Zipper (Group l Window))
addWindows _ _ (x, []) = return x
addWindows l _ (Nothing, as) = return . singletonZ $ G l (W.differentiate as)
addWindows l q (z, a:as) = do
  f <- runQuery q a
  let add z = insertDownZ a z
      z' = onFocusedZ (onZipper add) z
      z'' = f l z'
  addWindows l q (z'', as)
...
instance (LayoutClass l Window, LayoutClass l2 (Group l Window),
          GroupHook (Groups l l2 Window) l)
    => LayoutClass (Groups l l2) Window where
        ...
        runLayout ws@(W.Workspace _ _l z) r = 
            do l <- readapt z _l
...
type ModifySpec = forall l. ModSpec l
type ModSpec l = WithID l Window
