...
module ContribMod.Tabbed
    ( -- * Usage:
      -- $usage
    createTabs
    --  simpleTabbed, tabbed, addTabs, addTabsAlways
...
data DirectionShunt = U Word32 | D Word32 | L Word32 | R Word32
  deriving (Read,Show)
...
data TabbedDecoration a = Tabbed DirectionShunt TabbarShown deriving (Read, Show)
...
instance Eq a => DecorationStyle TabbedDecoration a where
    ...
    pureDecoration (Tabbed lc sh) wt ht _ s wrs (w,r@(Rectangle x y wh hh))
        ...
        where ws = filter (`elem` map fst (filter ((==r) . snd) wrs)) (S.integrate s)
              ...
              nx = n x wh
              ny = n y hh
              upperTab dy = Rectangle nx  y wid (fi ht + fi dy)
              lowerTab dy = Rectangle nx
                (y + fi (hh - ht) - fi dy) wid (fi ht + fi dy)
              leftTab dx = Rectangle x ny (fi wt + fi dx) hid
              rightTab dx = Rectangle (x + fi (wh - wt) - fi dx) ny
                              (fi wt + fi dx) hid
              numWindows = length ws
    shrink (Tabbed loc _ ) (Rectangle _ _ dw dh) (Rectangle x y w h)
        = case loc of
            (U dy) -> Rectangle x (y + fi dh - fi dy) w (h - dh + fi dy)
            (D dy) -> Rectangle x y w (h - dh + fi dy)
            (L dx) -> Rectangle (x + fi dw - fi dx) y (w - dw + fi dx) h
            (R dx) -> Rectangle x y (w - dw + fi dx) h
