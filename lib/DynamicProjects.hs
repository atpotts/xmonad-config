...
-- patched version of XMonad Actions DynamicProjects
-- 
module DynamicProjects
       ( -- * Overview
         -- $overview
...
dynamicProjectsLogHook = do
  ...
  unless (Just name == previousProject xstate) $ do
    ...
    case Map.lookup name (projects xstate) of
         Just pn -> activateProject pn
         Nothing -> do
             let p = defProject name
             updateProject p
             activateProject p
    case previousProject xstate >>= flip Map.lookup (projects xstate) of
         Just p -> clearProjectIfEmpty p
         Nothing -> return ()
...
--------------------------------------------------------------------------------
updateProject :: Project -> X ()
updateProject p = do
  ps <- XS.gets projects
  let ps' = Map.insert (projectName p) p
          $ Map.delete (projectName p) ps
...
switchProject p = do
  ...
  activateProject p
  clearProjectIfEmpty oldp

clearProjectIfEmpty oldp = do
  let name = projectName oldp
