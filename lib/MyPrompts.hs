...
myPrompts conf = [
          (noModMask, xK_semicolon, "", multiPrompt conf $ myPrompts conf ),
          (noModMask, xK_a, "Run or Raise", runOrRaisePrompt runConf),
          (noModMask, xK_w, "Go to Window", windowPrompt winConf Goto allWindows),
          (shiftMask, xK_w, "Bring Window to Master", windowPrompt winConf BringToMaster allWindows),
          (noModMask, xK_s, "Run(sh)", shellPrompt shellConf),
          (shiftMask, xK_s, "Run(term)", prompt ("urxvt -e") termConf)
          ]
...
multiPrompt conf list = inputPromptWithCompl newconf "" 
        ...
        where promptMap = M.fromList
              ...
              newconf = conf {promptKeymap = M.insert
                                             (noModMask, xK_space)
